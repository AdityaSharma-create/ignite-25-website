  ...
  const ScheduleTable: React.FC<{ title: string, data: any[] }> = ({ title, data }) => {
    ...
    // Find the first non-heading row to determine the table headers
    const firstDataRow = data.find(row => !row.isHeading);
    if (!firstDataRow) return null; // No data rows to display
  
    // Filter out the 'isHeading' property from the headers
    const headers = Object.keys(firstDataRow).filter(key => key !== 'isHeading');
          ...
          <table className="min-w-full text-sm text-left text-gray-300">
            ...
            <tbody>
              {data.map((row, index) => {
                if (row.isHeading) {
                  return (
                    <tr key={index} className="bg-gray-700 font-bold">
                      <td colSpan={headers.length} className="px-6 py-3 text-center text-white">
                        {row['MATCH NUMBER']}
                      </td>
                    </tr>
                  );
